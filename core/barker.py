...
def smell_test(
    ...
    if auth_type == "bearer":
        ...
        import base64
        import os
        creds = os.environ.get(api_key_env) or os.environ.get("BASIC_AUTH")
        if creds:
...
\t\tif bearer == "" {{ bearer = getEnv("{env_prefix}_BEARER_TOKEN", "") }}
\t\tif apiKey == "" {{ apiKey = getEnv("{api_key_env}", "") }}
\t\tif basicAuth == "" {{ basicAuth = getEnv("{api_key_env}", "") }}
\t}},
}}

func init() {{
\trootCmd.PersistentFlags().StringVar(&baseURL, "base-url", getEnv("{env_prefix}_BASE_URL", ""), "API base URL")
\trootCmd.PersistentFlags().StringVar(&bearer, "bearer", "", "Bearer token")
\trootCmd.PersistentFlags().StringVar(&apiKey, "api-key", "", "API key / token")
\trootCmd.PersistentFlags().StringVar(&basicAuth, "basic-auth", "", "Basic auth (username:password)")
\trootCmd.PersistentFlags().StringVar(&authType, "auth", getEnv("{env_prefix}_AUTH", "none"), "Auth: none, bearer, api_key_header, basic")
}}
...
function request(method, path, query, body, cb) {
  ...
  if (authType === "bearer" && bearer) opts.headers["Authorization"] = "Bearer " + bearer;
  else if (authType === "basic" && apiKey) opts.headers["Authorization"] = "Basic " + Buffer.from(apiKey).toString("base64");
  else if (authType === "api_key_header" && apiKey) opts.headers["__AUTH_HEADER__"] = apiKey;
